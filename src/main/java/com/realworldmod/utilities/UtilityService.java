...
/**
 * Server-lifecycle-scoped facade over a cached {@link UtilityState} per
 * player and {@link UtilityDatabase}, in the same open/cache/persist shape
 * as {@code PropertyService}/{@code BankService}. Billing itself goes
 * through {@link BankService}, so a household's power is only as reliable
 * as its owner's bank balance.
 */
public final class UtilityService {
    public static final long BILL_CENTS = 1000;
    public static final long BILLING_INTERVAL_TICKS = 20L * 60L * 5L;

    private final BankService bankService;
    private final Map<UUID, UtilityState> states = new HashMap<>();
    private UtilityDatabase database;
    private long lastBillingBucket = -1;

    public UtilityService(BankService bankService) {
        this.bankService = bankService;
    }

    public void open(Path dbFile) {
        database = new UtilityDatabase(dbFile);
        database.open();
        states.clear();
    ...
    public UtilityState getState(UUID ownerId) {
        return states.getOrDefault(ownerId, UtilityState.connected());
    }

    /** Attempts to pay off all accrued debt right now; returns whether it succeeded. */
    public boolean payNow(UUID ownerId) {
        UtilityState previous = getState(ownerId);
        boolean paid = previous.unpaidCents() <= 0
                || bankService.withdraw(ownerId, previous.unpaidCents()).isPresent();
        setState(ownerId, UtilityBillingMath.afterManualPayment(previous, paid));
        return paid;
    }
    ...
    public List<UUID> tick(long currentTick, Iterable<UUID> onlinePlayerIds) {
        ...
        for (UUID playerId : onlinePlayerIds) {
            UtilityState previous = getState(playerId);
            long amountOwed = previous.unpaidCents() + BILL_CENTS;
            boolean paid = bankService.withdraw(playerId, amountOwed).isPresent();
            UtilityState next = UtilityBillingMath.afterBillingAttempt(previous, BILL_CENTS, paid);
            setState(playerId, next);
            if (previous.powerConnected() && !next.powerConnected()) {
    ...
    private void setState(UUID ownerId, UtilityState state) {
